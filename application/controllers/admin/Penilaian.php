...
class Penilaian extends CI_Controller {
	...
	public function new()
	{
		...
		if ($this->input->method() === 'post') {
			...
			if ($ada) {
				$this->session->set_flashdata('message', 'Siswa sudah terdata!');
				return redirect('admin/penilaian/new');
			}
		...
		$dropdownSiswa[null] = '---Pilih Siswa---';
		if ($siswa) {
			foreach ($siswa as $s) {
				$dropdownSiswa[$s->id] = $s->nis . " - " .$s->nama;
			}
		}
